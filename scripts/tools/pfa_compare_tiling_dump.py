#!/usr/bin/env python3
# Compare C++ PFA_TILING_DUMP logs with pfa_tiling_sim.py JSON output.
...
SUMMARY_TAG = "[PFA_TILING_DUMP][summary]"
CORE_RANGE_TAG = "[PFA_TILING_DUMP][coreRange]"
...
def parse_cpp_dump(path: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "coreRangeCount": len(core_ranges),
    }
...
def compare_summary(cpp_summary: Dict[str, Any], py: Dict[str, Any], float_tol: float) -> List[Diff]:
    mapping = [
        ...
        ("usedCoreNum", "splitCore.cubeUsedCores"),
        ("actualCoreNums", "splitCore.actualCoreNums"),
        ("batchSize", "normalizedInput.batch_size"),
        ("headNumSize", "normalizedInput.head_num_size"),
        ("seqSize", "normalizedInput.seq_size"),
        ("seqInnerSize", "normalizedInput.seq_inner_size"),
        ("singleProcessSOuterSize", "tiling.Souter"),
        ("cubeSOuterSize", "splitCore.souter_for_split"),
        ("singleProcessSInnerSize", "tiling.Sinner"),
        ("softmaxOuterSize", "tiling.SoftmaxSouter"),
        ("splitS2", "tiling.splitS2"),
        ("totalBlockNumsOneHead", "splitCore.total_block_nums_one_head"),
        ("multiSmaxsInnerLoopTimes", "splitCore.multiSmaxsInnerLoopTimes"),
    ...
    for cpp_key, py_path in mapping:
        try:
            cpp_value = cpp_summary[cpp_key]
            py_value = get_path(py, py_path)
            compare_one(diffs, f"summary.{cpp_key} <-> {py_path}", cpp_value, py_value, float_tol)
...
def normalize_cpp_core_range(item: Dict[str, Any]) -> Dict[str, Any]:
    core_id = item.get("cubeCore", item.get("core"))
    return {
        "cube_core": core_id,
        "aiv_core_pair": item.get("aivCorePair"),
        "n_start": item.get("nStart"),
        "n_end": item.get("nEnd"),
        "batch_start": item.get("batchStart"),
        "batch_end": item.get("batchEnd"),
        "souter_start": item.get("sOuterStart"),
        "souter_end": item.get("sOuterEnd"),
        "bn_start_idx": item.get("bnStartIdx"),
        "bn_end_idx": item.get("bnEndIdx"),
        "gs1_start_idx": item.get("sparseStartIdx"),
        "gs1_end_idx": item.get("sparseEndIdx"),
...
def compare_core_ranges(cpp_ranges: List[Dict[str, Any]], py: Dict[str, Any], float_tol: float) -> List[Diff]:
    py_ranges = py.get("splitCore", {}).get("coreRanges", [])
    diffs: List[Diff] = []
    compare_one(diffs, "coreRanges.length", len(cpp_ranges), len(py_ranges), float_tol)
    ...
    for idx, cpp_item in enumerate(cpp_ranges[: len(py_ranges)]):
        ...
        for field in fields:
            if field == "aiv_core_pair" and cpp_norm.get(field) is None and "aiv_core_pair" not in py_item:
                continue
            compare_one(
                diffs,
                f"coreRanges[{idx}].{field}",
                cpp_norm.get(field),
                py_item.get(field),
                float_tol,
            )
...
def compare_task_arrays(cpp_ranges: List[Dict[str, Any]], py: Dict[str, Any], float_tol: float) -> List[Diff]:
    cpp_blocks = [normalize_cpp_core_range(item).get("task_blocks") for item in cpp_ranges]
    py_blocks = py.get("splitCore", {}).get("coreTaskBlocks", [])
    diffs: List[Diff] = []
    compare_one(diffs, "splitCore.coreTaskBlocks.length", len(cpp_blocks), len(py_blocks), float_tol)
    for idx, (cpp_value, py_value) in enumerate(zip(cpp_blocks, py_blocks)):
        compare_one(diffs, f"splitCore.coreTaskBlocks[{idx}]", cpp_value, py_value, float_tol)

    candidate = py.get("splitCore", {}).get("candidateCoreTaskBlocks")
    candidate_cores = py.get("splitCore", {}).get("candidate_cube_cores")
    if candidate is not None and candidate_cores is not None:
        expected = cpp_blocks + [0] * max(0, int(candidate_cores) - len(cpp_blocks))
        compare_one(diffs, "splitCore.candidateCoreTaskBlocks.length", len(expected), len(candidate), float_tol)
        for idx, (cpp_value, py_value) in enumerate(zip(expected, candidate)):
            compare_one(diffs, f"splitCore.candidateCoreTaskBlocks[{idx}]", cpp_value, py_value, float_tol)
    return diffs
...
def build_report(cpp: Dict[str, Any], py: Dict[str, Any], float_tol: float) -> Dict[str, Any]:
    ...
    diffs.extend(compare_task_arrays(cpp["coreRanges"], py, float_tol))
...
def print_text_report(report: Dict[str, Any], show_ok: bool) -> None:
    print("PFA tiling dump compare:", "PASS" if report["ok"] else "FAIL")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare PFA C++ tiling dump logs with pfa_tiling_sim.py JSON.")
    parser.add_argument("--cpp-log", required=True, help="Log file containing [PFA_TILING_DUMP] lines.")
    parser.add_argument("--py-json", required=True, help="JSON produced by scripts/tools/pfa_tiling_sim.py.")
